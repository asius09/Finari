...
export const filterTransactionByPeriod = (
  ...
  const startOfWeek = format(new Date(now.setDate(now.getDate() - now.getDay())), "yyyy-MM-dd");
  const startOfLastWeek = format(new Date(now.setDate(now.getDate() - 7)), "yyyy-MM-dd");
  ...
  switch (period) {
    case "Today":
      return transaction.filter(t => format(new Date(t.date), "yyyy-MM-dd") === today);
    case "Yesterday":
      return transaction.filter(t => format(new Date(t.date), "yyyy-MM-dd") === yesterday);
    case "This Week":
